...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        # 排序后得到与每个点云 xyz1 中每个点最接近的 3 个点在 xyz2 中的索引和距离。
        # 使用这些距离计算权重，并对点云 2 (points2) 中的特征进行加权平均，以得到每个点的插值特征 interpolated_points。

        # 如果 points1 存在，将 points1 和 interpolated_points 沿最后一个维度拼接，以便在特征上进行融合。
        # 如果 points1 为 None，则仅使用插值后的特征。
        if points1 is not None:
            # points1 = points1.permute(0, 2, 1)
        ...
        return new_points   # 最后，将数据转换回 [B, N, D] 的形状。
...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        idx = knn_point(self.group_size, xyz, center)   # idx -> B, G, M
