...
class H_Encoder(nn.Module):
    ...
    def forward(self, neighborhoods, centers, idxs, eval=False):
        ...
        for i in range(len(neighborhoods) - 1, 0, -1):  # 倒序， i 取的是 i - 1 中center的点 
            ...
            # exit(-1)
            idx_masked = ~(bool_masked_pos[-1].reshape(-1).unsqueeze(-1)) * idx # B * G, -1 with B * G, -1
            # 这里留下了center 
            # idx_masked -> B * G, M
        ...
        # for patch matching
        # pos_emd_mask = self.encoder_pos_embeds[i](centers[-1][bool_masked_pos[-1]]).reshape(B, -1, C)
        ...
        for i in range(len(centers)):
            ...
            x_vis = self.encoder_blocks[i](x_vis, pos, mask_vis_att)        # encoder block forward NOTE
...
class Point_M2AE(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.pred_pos_proj1 = nn.Sequential( # input B, M, C  
        ...
        # self.pred_point_proj1 = nn.Sequential( # input B, M, C  
        #     nn.Linear(self.trans_dim, self.trans_dim),
        #     nn.LayerNorm(self.trans_dim),
        #     nn.ReLU(inplace=True),
        #     nn.Linear(self.trans_dim, self.trans_dim),
